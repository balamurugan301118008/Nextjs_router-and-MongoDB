...
export default function UserHome() {
    ...
    const handleDeleteTask = (e) => {
        axios.post(`http://localhost:4000/delete`, { deleteId: e.target.id })
            .then(res => {
    ...
    return (
        <div>
            ...
            <div className='d-flex  justify-content-start align-items-center gap-5'>
                <Button type="primary" className='bg-success ms-5' onClick={showModal}>
                    Add task
                </Button>
                        ...
                        filteredList.map((item, index) =>
                            <div key={index} className='taskContainer'>
                                ...
                                <div className='d-flex justify-content-start gap-3'>
                                    ...
                                    <button type="button" id={item._id} onClick={handleDeleteTask} className="btn btn-outline-danger btn-sm">Delete</button>
                                    <Link href={`/userHome/editTask/${item._id}`}><button id={item._id} className='btn btn-outline-success btn-sm'>Edit</button></Link>
                                </div>
                            </div>
                        )) : (
                        <div>
