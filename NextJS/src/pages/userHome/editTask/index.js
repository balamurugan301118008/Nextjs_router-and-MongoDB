...
function EditTask() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:4000/userHome/editTask/${taskId}`)
            .then((response) => {
    ...
    const handleUpdateTask = (e) => {
        ...
        // }
        // else {
        axios.post(`http://localhost:4000/userHome/editTask/${taskId}`, { taskName: taskName, description: description, status: status })
            .then(res => {
                if (res.data.Status == "Success") {
                    notification.success({ description: 'Your Task Successfully Updated' })
                    naviagate.push("/userHome")
                }
            })
    ...
    return (
        <div>
            <div className='d-flex justify-content-around p-3'>
                ...
                <Link href='/userHome'><button className='btn btn-outline-primary'>Back to UserHomePage</button></Link>
            </div>
