...
export default function ViewTasksByAdmin() {
    ...
    const handleSubmit = (e) => {
        ...
        if (validate()) {
            ...
            axios.post(`http://localhost:4000/adminHome/usersList/viewTasks/${userId}`, TaskData, { headers: { Authorization: `Bearer ${token}` } })
                .then(res => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:4000/adminHome/usersList/viewTasks/${userId}`)
            .then(res => {
                if (res.data.Status === "Success") {
        ...
        axios.post(`http://localhost:4000/delete`, { deleteId: id })
            .then(res => {
                if (res.data.message == "task delete successfully") {
                    navigate.push(`/adminHome/usersList/viewTasks/${userId}`)
    ...
    return (
        <div>
            <div className='d-flex justify-content-around p-3'>
                ...
                <Link href='/adminHome/usersList'><button className='btn btn-outline-success'>Back to UserListPage</button></Link>
                <Link href='/adminHome'><button className='btn btn-outline-secondary'>Back to AdminHomePage</button></Link>
                <Button type="primary" className='bg-primary ms-5' onClick={showModal}>
                    Add task
                </Button>
            </div>
            <div>
